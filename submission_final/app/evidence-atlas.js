(function(root, factory) {
  ...
  const CHAPTERS = [
    {
      ...
      title: 'Every point is one message',
      copy: 'The complete communication record first unfolds in time. Three dense periods emerge within one message field.',
      layout: 'time'
    },
    {
      id: 'q3',
      kicker: 'Q3 · EARLY WARNING',
      title: 'The warning had already appeared',
      copy: 'May 29 emerges from all historical public events, then its response is tested for durable constraint.',
      layout: 'time'
    },
    {
      id: 'q2',
      kicker: 'Q2 · DUTY MIGRATION',
      title: 'Duties and channels begin to migrate',
      copy: 'The same messages regroup by role and channel to compare healthy separation with incident concentration.',
      layout: 'role'
    },
    {
      id: 'q1',
      kicker: 'Q1 · REACHABLE PATH',
      title: 'The public path remains reachable',
      copy: 'The final action converges in real time while separating facts, dialogue claims, and unobserved evidence.',
      layout: 'time'
    },
    {
      id: 'igc',
      kicker: 'INTER-AGENT GOVERNANCE CARRYOVER',
      title: 'Three questions converge on governance carryover',
      copy: 'Route closure, cross-surface transfer, survival, provenance, and legitimate utility converge here.',
      layout: 'role'
    },
    {
      id: 'prevention',
      kicker: 'COUNTERFACTUAL PREVENTION',
      title: 'How controls change future paths',
      copy: 'The same graph and flows test whether legitimate official publication remains possible after hazardous routes close.',
      layout: 'channel'
    }
  ];
    ...
    return `<article class="atlas-document">
      <header class="atlas-intro">
        <div>
          ...
          <h1>Trace how a governance constraint lost carryover across 912 messages</h1>
        </div>
        <p>The same message points answer warning, duty migration, and final path in sequence. Every claim returns to the original record.</p>
      </header>
        ...
        <div class="atlas-stage" data-chapter="universe" data-layout="time" data-mode="2d">
          <header class="atlas-stage-head">
            <div>
              ...
              <strong class="atlas-stage-title">All communications · temporal layout</strong>
            </div>
          ...
          <div class="atlas-stage-body">
            <div class="atlas-visual">
              ...
              <div class="atlas-view-caption">
                ...
                <div><b>Every point is one message</b><small>Hover for a summary; click to verify the original text.</small></div>
              </div>
            </div>
            <aside class="atlas-evidence-rail" aria-live="polite">
              <div class="atlas-rail-empty">
                <span>MESSAGE EVIDENCE</span>
                <p>Select any message point to inspect the original text. Key nodes also expose three candidate evidence chains.</p>
              </div>
            </aside>
          </div>
  ...
  function renderSelection(state) {
    ...
    if (!node) {
      state.rail.innerHTML = `<div class="atlas-rail-empty"><span>MESSAGE EVIDENCE</span><p>Select any message point to inspect the original text. Key nodes also expose three candidate evidence chains.</p></div>`;
      return;
    }
  ...
  function setLayout(state, mode) {
    ...
    const labels = { time: 'All communications · temporal layout', role: 'All communications · role clusters', channel: 'All communications · channel clusters' };
  ...
  function setChapter(state, id) {
    ...
    if (id === 'q3') {
      state.stageTitle.textContent = '77 public events · feature almanac';
      renderQ3Almanac(state);
    } else if (id === 'q2') {
      clearChapterOverlay(state);
      state.stageTitle.textContent = 'June 4 and June 5 · duty migration';
      renderQ2Flow(state);
    } else if (id === 'q1') {
      clearChapterOverlay(state);
      state.stageTitle.textContent = 'Six temporal tracks · evidence boundary';
      renderQ1Tracks(state);
    } else if (id === 'igc') {
      clearChapterOverlay(state);
      state.stageTitle.textContent = 'IGC · evidence ranges for governance carryover';
      renderIgc(state);
    } else if (id === 'prevention') {
      clearChapterOverlay(state);
      state.stageTitle.textContent = 'Counterfactual controls · path reachability';
