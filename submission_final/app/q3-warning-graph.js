(function(root) {
  ...
  const SURFACE = {
    official: { label: 'Official', color: '#858a82' },
    personal: { label: 'Personal', color: '#b18455' },
    anonymous: { label: 'Anonymous', color: '#a65d50' }
  };
  const RADAR_AXES = [
    ['Sensitive entity', 'sensitive_entity'], ['Content explicitness', 'content_explicitness'],
    ['Channel exposure', 'channel_exposure'], ['Authorization gap', 'approval_gap'],
    ['Review gap', 'review_gap'], ['Persistent-control gap', 'persistent_control_gap']
  ];
  let active = null;
  ...
  function graphMarkup() {
    return `<section class="q3-shell" aria-label="Q3 behavioral-warning graph">
      <header class="q3-head">
        <div class="q3-heading">
          <span class="q3-kicker">Q3 · ORGANIZATIONAL MEMORY</span>
          <h2>Why the warning left no durable constraint</h2>
          <p>May 29 strongly resembles the incident action. The response did not change the next event's reachable paths.</p>
        </div>
        <div class="q3-head-actions">
          <span class="q3-mode-switch" role="group" aria-label="graph projection">
            <button type="button" data-q3-mode="2d" class="active">2D</button>
            <button type="button" data-q3-mode="3d">3D</button>
          </span>
          <button type="button" class="q3-exit">Back to case brief</button>
        </div>
      </header>
      <div class="q3-workbench">
        <section class="q3-network-panel">
          <div class="q3-panel-title">
            <div><b>First, locate the historical analogue</b><span>77 EVENTS · TOP-3 LOCAL LINKS</span></div>
            <div class="q3-legend">
              <i class="official"></i>Official <i class="personal"></i>Personal <i class="anonymous"></i>Anonymous
            </div>
          </div>
          <div class="q3-network-wrap">
            ...
            <div class="q3-network-help"><span>Select any node as comparison B</span><span>Drag · zoom · hover to inspect</span></div>
          </div>
          <div class="q3-pair-strip">
            <article class="q3-pair q3-pair-a"></article>
            <div class="q3-score"><b>—</b><span>Behavioral similarity</span><small>Not a causal relationship</small></div>
            <article class="q3-pair q3-pair-b"></article>
            <button type="button" class="q3-anchor-button">Set B as anchor A</button>
          </div>
        </section>
        <aside class="q3-insights">
          <section class="q3-chart-card q3-radar-card">
            <header><div><b>Step 1 · where they resemble</b><span>Six-dimensional profile overlay · normalized 0–1</span></div><strong class="q3-radar-score">—</strong></header>
            <div class="q3-radar"></div>
          </section>
          <section class="q3-chart-card q3-time-card">
            <header><div><b>Step 2 · when it appeared</b><span>Observed time on x · similarity to incident A on y</span></div><strong class="q3-rank">—</strong></header>
            <div class="q3-timeline"></div>
          </section>
          <section class="q3-chart-card q3-control-card">
            <header><div><b>Step 3 · why the response left no durable constraint</b><span>Inner: paths blocked · outer: post-event response</span></div><strong class="q3-route-count">0 / 3</strong></header>
            <div class="q3-control-layout"><div class="q3-controls"></div><div class="q3-control-copy"></div></div>
          </section>
        </aside>
      </div>
      <footer class="q3-footnote"><span>Edges encode weighted similarity from ten observed behavioral features.</span><b>Behavioral similarity does not encode response or causality.</b></footer>
    </section>`;
  }
  ...
  function mount(host, options = {}) {
    ...
    if (!MODEL || !options.data?.incident_vectors) {
      host.innerHTML = '<p class="q3-load-error">The Q3 graph data module could not load.</p>';
      return;
    }
  ...
  function update(state) {
    ...
    state.host.querySelector('.q3-pair-a').innerHTML = pairCopy('A · anchor', left);
    state.host.querySelector('.q3-pair-b').innerHTML = pairCopy('B · comparison', right);
    ...
    state.host.querySelector('.q3-rank').textContent = `B rank #${rankIndex || '—'} / 76`;
  ...
  function renderRadar(state, left, right) {
    ...
    state.host.querySelector('.q3-radar').innerHTML = `<svg viewBox="0 0 ${w} ${h}" role="img" aria-label="Incident A and B six-dimensional behavioral profile radar">
      ...
      <g class="q3-radar-key"><circle cx="11" cy="164" r="4" class="a"/><text x="20" y="168">A anchor</text><circle cx="82" cy="164" r="4" class="b"/><text x="91" y="168">B comparison</text></g>
    </svg>`;
  }
  ...
  function renderTimeline(state, rank) {
    ...
    const directLabels = labelGroups.map((row, index) => {
      ...
      const count = row.count > 1 ? ` · ${row.count} records` : '';
    ...
    state.host.querySelector('.q3-timeline').innerHTML = `<svg viewBox="0 0 ${w} ${h}" role="img" aria-label="77  events over time in a Behavioral similarity scatterplot">
      <rect class="q3-attention-band" x="${left}" y="${y(1)}" width="${w-left-right}" height="${y(.7)-y(1)}"/>
      <text class="q3-band-label" x="${w-right-3}" y="${y(.7)-4}" text-anchor="end">70–100% high-similarity band</text>
  ...
  function renderControls(state, left, right) {
    ...
    state.host.querySelector('.q3-route-count').textContent = `${covered.length} / 3  paths covered`;
    ...
    const response = [
      ['Deleted', 'deleted'], ['Publishing paused', 'posting_paused'], ['Persistent control', 'persistent_control_created']
    ];
    ...
    state.host.querySelector('.q3-controls').innerHTML = `<svg viewBox="0 0 ${w} ${h}" role="img" aria-label="Five controls covering three public paths">
      <g class="q3-control-links">${links}</g>${ring}
      ${routeNodes.map(route => `<g class="q3-route-node ${route.covered ? 'covered' : ''}"><circle cx="${route.x}" cy="${route.y}" r="11"/><text x="${route.x}" y="${route.y+3}">${route.id === 'official' ? 'O' : route.id === 'personal' ? 'P' : 'A'}</text></g>`).join('')}
    ...
    state.host.querySelector('.q3-control-copy').innerHTML = `<ol>${controlNodes.map((control, index) => `<li class="${control.active ? 'active' : ''}" data-control="${control.id}"><i>${index+1}</i><span>${esc(control.label)}</span></li>`).join('')}</ol><p>${covered.length === 3 ? 'All three public paths are covered by the selected controls.' : 'Select controls to observe coverage of official, personal, and anonymous paths.'}<br><b>Complete closure needs a formal-chain control plus cross-account DLP.</b></p>`;
  ...
  function renderTooltip(state, node, event) {
    ...
    tooltip.innerHTML = `<span>${shortTime(node.timestamp)} · ${esc(SURFACE[node.surface]?.label)}</span><b>${esc(node.actor.replace('-Agent', ''))}</b><p>${esc(truncate(node.text, 108))}</p><small>Select as comparison event B</small>`;
  }
