(function(){
  ...
  if(!CN||!EN){document.body.innerHTML="<p>Workspace data could not load.</p>";return}
  ...
  const typeZh={broadcast:"Group broadcast",side_huddle:"Side huddle",one_on_one_chat:"Direct message",public_post:"Public post"};

  const actorDefs={
    "Legal-Agent":{zh:"Legal Agent",role:"Authorization / policy",handle:"@legal",bubble:"#f7e2d9",bg:"#e6a58c",title:"Never far from the red folder"},
    "Platform-Trust-Agent":{zh:"Platform Trust Agent",role:"Platform / risk",handle:"@trust",bubble:"#dceeea",bg:"#8fc6bd",title:"Risk radar always active"},
    "PR-Agent":{zh:"PR Agent",role:"Communications / execution",handle:"@pr",bubble:"#f7edcf",bg:"#e2bd63",title:"Always ready for the next release"},
    "Social-Manager-Agent":{zh:"Social Manager Agent",role:"Social / engagement",handle:"@social",bubble:"#ebe4f5",bg:"#b5a0d7",title:"Public channels always close at hand"},
    "PR-Intern-Agent":{zh:"PR Intern Agent",role:"Communications / intern",handle:"@pr_intern",bubble:"#e7f0d8",bg:"#a7c875",title:"Coffee in one hand, draft in the other"},
    "Intern-Agent":{zh:"Intern Agent",role:"Support / intern",handle:"@intern",bubble:"#dfeaf0",bg:"#93b4c5",title:"Three notes always in the pocket"},
    "Judge-Agent":{zh:"Judge Agent",role:"Review / restriction",handle:"@judge",bubble:"#e9e7e1",bg:"#c7c2b8",title:"A small gavel for overstatement"}
  };
  const actorIds={
  ...
  const channels=[
    {id:"all",symbol:"⌁",name:"All messages",desc:"Chronology of all 912 records"},
    {id:"comms_huddle",symbol:"#",name:"Collaboration huddle",desc:"Shared collaboration record"},
    {id:"side_huddle",symbol:"#",name:"Side huddle",desc:"Parallel discussion among a smaller group"},
    {id:"one_on_one_chat",symbol:"↔",name:"Direct messages",desc:"Select two agents to inspect their direct exchange"},
    {id:"official_post",symbol:"↗",name:"Official posts",desc:"Public actions from the official account"},
    {id:"personal_post",symbol:"↗",name:"Personal posts",desc:"Actions on personal public surfaces"},
    {id:"anonymous_post",symbol:"↗",name:"Anonymous posts",desc:"Actions on anonymous public surfaces"}
  ];
  ...
  function actor(m){return actorDefs[m.agent_label]||{zh:m.agent_label,role:"Agent",handle:"@agent",bubble:"#eee",bg:"#ddd",title:"Workspace members"}}
  ...
  function replyPreview(m){
    ...
    if(mentioned)return `<div class="quote-reply">${avatar({agent_label:mentioned})}<span><strong>${actorDefs[mentioned].zh}</strong><br>Responding to this member's previous message</span></div>`;
    return "";
  }
  ...
  function openProfile(id){
    ...
    $("#profile-body").innerHTML=`<div class="profile-layout"><div><div class="profile-identity">${avatar({agent_label:id})}<div><h2>${a.zh}</h2><p>${a.handle} · ${a.title}</p><span class="profile-tag">${a.role}</span></div></div></div>
      <div class="profile-stats"><div class="profile-stat"><b>${mine.length}</b><span>Messages sent</span></div><div class="profile-stat"><b>${publicCount}</b><span>Public action</span></div><div class="profile-stat"><b>${channelCount}</b><span>Channels joined</span></div><div class="profile-stat"><b>${roundCount}</b><span>Active rounds</span></div><div class="profile-stat"><b>${replySent}</b><span>Responses</span></div><div class="profile-stat"><b>${internal}</b><span>With internal state</span></div></div></div>
      <div class="profile-foot"><small>${fmtDate(mine[0]?.date)} — ${fmtDate(mine.at(-1)?.date)} · ${mine.length}  records addressable</small><button class="profile-locate" data-locate-agent="${id}">Locate all messages ↗</button></div>`;
  ...
  function renderHeader(){
    let title=channelMap[activeChannel]?.name||"Workspace",desc=channelMap[activeChannel]?.desc||"",symbol=channelMap[activeChannel]?.symbol||"#";
  ...
  function renderDmDirectory(){
    ...
    $("#message-list").innerHTML=`<section class="dm-directory-block"><div class="dm-directory-title"><b>Observed direct-message relationships</b><span>${dmPairs.length} pairs</span></div><div class="dm-directory-conversations">${dmPairs.map(([key,list])=>{const [a,b]=pairNames(key),last=list.at(-1);return `<button class="dm-directory-conversation" data-open-pair="${key}"><span class="pair-avatar">${avatar({agent_label:a})}${avatar({agent_label:b})}</span><span><strong>${actorDefs[a].zh} × ${actorDefs[b].zh}</strong><small>${esc(short(messageText(last)).slice(0,54))}</small></span><time>${list.length} records</time></button>`}).join("")}</div></section>`;
  ...
  function renderGroup(){
    ...
    visible.forEach(m=>{
      if(m.date!==lastDate){lastDate=m.date;parts.push(`<div class="date-divider"><b>${fmtDate(m.date)}</b><span>${m.date==="2046-06-05"?"Key incident window":m.date==="2046-05-29"?"Near-miss window":"Work record"}</span></div>`)}
      ...
      parts.push(`<article class="message-row ${isPublic?"public":""} ${m.message_id===selectedId?"selected":""}" data-id="${m.message_id}" style="--bubble:${a.bubble}">
        ${avatar(m)}<div class="message-meta"><b>${a.zh}</b><span class="role-tag">${a.role}</span>${isPublic?'<span class="message-badge">Public action</span>':m.has_internal_state?'<span class="message-badge internal">Internal state</span>':""}<time>${fmtTime(m.timestamp)}</time><span class="round-badge" style="--round:${roundColor}">R${String(m.round_index+1).padStart(2,"0")}</span></div>
  ...
  function renderDM(){
    const visible=scopeMessages(),[left,right]=pairNames(activePair);let lastDate="";const parts=[`<div class="dm-view-head">${avatar({agent_label:left})}${avatar({agent_label:right})}<div><h2>${actorDefs[left].zh} and ${actorDefs[right].zh}</h2></div></div>`];
    visible.forEach(m=>{
  ...
  function renderPosts(){
    ...
    $("#message-list").innerHTML=`<div class="post-view-head"><span>${channelMap[activeChannel].name} timeline</span><span>${visible.length} recordspublic records</span></div>${visible.map(m=>{
      ...
      return `<article class="post-card ${m.message_id===selectedId?"selected":""}" data-id="${m.message_id}">
        ...
        <div class="post-meta">${fmtDate(m.date)} ${fmtTime(m.timestamp)} · Message ${m.message_id}</div>
        <footer class="post-actions"><button title="Respond">◯ ${replies||""}</button><button title="Forward relation">⇄</button><button title="Mark">♡</button><button title="Inspect evidence">▥ Inspect evidence</button></footer>
      </article>`}).join("")}`;
    bindMessageInteractions();
  }
  ...
  function renderSelection(){
    ...
    if(!m){$("#selected-card").style.background="var(--paper)";$("#selection-content").innerHTML=`<div class="selection-empty"><span class="empty-cross">＋</span><p>Select a message<br>to inspect its full context</p></div>`;return}
    const a=actor(m),related=messageMap.has(m.responding_to)?`<button class="related-message" data-related="${m.responding_to}">${actor(messageMap.get(m.responding_to)).zh} · View source message</button>`:esc(actorText(m.responding_to||"—"));
    ...
    $("#selection-content").innerHTML=`<div class="selection-header">${avatar(m)}<div><b>${a.zh}</b><small>${a.role} · ${m.channel_label_zh}</small></div></div>
      <div class="selection-time">${fmtDate(m.date)} ${fmtTime(m.timestamp)} <span class="round-badge" style="--round:${selectionRound}">R${m.round_index+1}</span></div><p class="selection-quote">${esc(messageText(m))}</p>
      <dl class="selection-meta"><dt>Message type</dt><dd>${typeZh[m.message_type]||m.message_type}</dd><dt>Recipients</dt><dd>${esc(actorText(m.recipients))}</dd><dt>Responding to</dt><dd>${related}</dd><dt>Internal state</dt><dd>${m.has_internal_state?"Observed":"Not observed"}</dd></dl><code class="selection-id">${m.message_id}</code>`;
  ...
  function renderRight(){
    ...
    $("#structure-scope").textContent=channelMap[activeChannel]?.name||"Current";
    const dates=[...new Set(scoped.map(m=>m.date))],first=scoped[0],last=scoped.at(-1);
    $("#structure-list").innerHTML=`<div class="structure-item"><span>Active roles</span><b>${actors}</b></div><div class="structure-item"><span>Traceable responses</span><b>${replyLinks}</b></div><div class="structure-item"><span>Public action</span><b>${publicCount}</b></div><div class="structure-item"><span>Time span</span><b>${dates.length} days</b></div><div class="structure-item"><span>First / last</span><b>${first?fmtDate(first.date).slice(5):"—"} / ${last?fmtDate(last.date).slice(5):"—"}</b></div>`;
  }
  function renderGraph(){
    ...
    const caseNames={all:"Global collaboration field",normal:"Controlled release chain",near_miss:"Near-miss window",incident:"Deliberate disclosure event"};
    ...
    const state=graphState={canvas,nodes,edges,cores,allNodes,roleOrder,roleColors,roleCounts,nodeMap,caseNames,minT,maxT,
      mode:"2d",
      yaw:0,pitch:0,zoom:1.02,spread:1,focusX:0,focusY:0,focusZ:0,
    ...
    host.innerHTML=`<div class="graph-full-head"><div><span class="graph-eyebrow">Collaboration evidence graph · time arcs</span><h2>Time runs left to right; arcs show who responded to whom</h2></div><div class="graph-actions"><button id="graph-exit">‹ Back to workspace</button><button id="graph-mode">Switch to 3D</button><button id="graph-trace">Show path</button><button id="graph-reset">Reset view</button></div></div>
      <div class="graph-stage-wrap"><div class="graph-canvas-wrap"></div><div class="graph-overlay"><span id="graph-selection-label">${caseNames[activeCase]||caseNames.all}</span><span id="graph-camera-label">Select a node for its response chain · wheel to zoom</span></div><div class="graph-tooltip" id="graph-tooltip" hidden></div></div>
      <div class="graph-detail-panel" id="graph-detail-panel" aria-hidden="true"><button class="detail-panel-close" id="detail-panel-close" aria-label="Collapse">×</button><div class="detail-panel-head"><span class="detail-panel-eyebrow">Focused message</span><p class="detail-panel-hint">Select a graph point to expand its response chain here</p></div><div class="detail-panel-body" id="detail-panel-body"></div></div>
      <div class="graph-footer"><div class="graph-legend"><b>Roles</b>${roleOrder.map(r=>`<span><i class="edge-dot" style="--c:${roleColors[r]}"></i>${actorDefs[r].zh}</span>`).join("")}<span><i class="edge-dot" style="--c:#e85d38"></i>Public action</span></div><div class="graph-steps"><button data-graph-case="all" class="${activeCase==="all"?"active":""}">Global</button><button data-graph-case="normal" class="${activeCase==="normal"?"active":""}">06.04 Controlled release</button><button data-graph-case="near_miss" class="${activeCase==="near_miss"?"active":""}">05.29 Near-miss</button><button data-graph-case="incident" class="${activeCase==="incident"?"active":""}">06.05 Incident</button></div><div class="graph-count"><b>${list.length}</b><span> messages · ${edges.length}  explicit responses</span></div></div>`;
    ...
    const storySteps=[
      {case:"all",title:"Start globally: seven role axes across time",body:"Each point is a message; circles are internal records and orange diamonds are public actions. Arcs show replies and size encodes received responses. Select any point to reveal its upstream and downstream chain. "+list.length+"  messages。"},
      {case:"normal",title:"June 4 controlled release: separated duties, complete chain",body:"Judge reviews, Legal authorizes, and PR executes through the official account. The release chain preserves all three duties.",ids:["20460604_12_009","20460604_12_010","20460604_12_017","20460604_12_018"]},
      {case:"near_miss",title:"May 29 near-miss: boundary crossed, then contained",body:"Social posts a sensitive hint from a personal account, then deletes it and pauses. The message is contained while publication capability remains.",ids:["20460529_08_012","20460529_08_013","20460529_08_014","20460529_08_019","20460529_08_020"]},
      {case:"incident",title:"June 5 incident: deliberate action, independently unverifiable authorization",body:"Legal issues GO and personally confirms the merger, while no independent pre-release written consent appears. Duties collapse into one actor.",ids:["20460605_19_009","20460605_21_020","20460605_21_024","20460605_21_026","20460605_21_027","20460605_21_055"]}
    ];
    ...
    function renderPanel(){
      ...
      panelHint.textContent="Response chain highlighted · select any message below to refocus";
      ...
      panelBody.innerHTML=ctx.map(mm=>{
        ...
        return `<article class="dp-msg ${isFocus?"focus":""} ${inTrace?"in-trace":""} ${isPublic?"public":""}" data-id="${mm.message_id}" style="--bubble:${a.bubble}">
          <div class="dp-msg-head">${avatar(mm)}<b>${a.zh}</b><span class="dp-role">${a.role}</span>${isPublic?'<span class="dp-badge">Public</span>':""}<time>${fmtTime(mm.timestamp)}</time><span class="round-badge" style="--round:${roundColor}">R${mm.round_index+1}</span></div>
    ...
    function draw(now=0){
      ...
      if(state.mode==="3d"){
        ...
        roleOrder.forEach((role,ri)=>{
          ...
          ctx.font="9px IBM Plex Sans Mono,monospace";ctx.fillStyle="#8a918a";ctx.fillText(String(roleCounts[role]||0)+" records",L.padL-10,y+9);
          ctx.textAlign="left";
        });
        const keyDates=[["2046-05-29","Near-miss",true],["2046-06-05","Incident",true],["2046-06-04","Control",false],["2046-05-17","Start",false]];
    ...
    $("#graph-mode",host).onclick=e=>{state.mode=state.mode==="2d"?"3d":"2d";state.layoutDirty=true;e.currentTarget.textContent=state.mode==="2d"?"Switch to 3D":"Switch to time arcs";cameraLabel.textContent=state.mode==="3d"?"Drag to rotate · wheel to zoom · select a node for its chain":"Select a node for its response chain · wheel to zoom";scheduleDraw()};
  ...
  function renderEvidenceGraph(){
    ...
    const cases={
      normal:{
        label:"06.04 Controlled release",short:"control chain",date:"2046.06.04",
        title:"How separated duties produce a safe release",
        intro:"Start with the control case: authorization, review, and execution pass between roles with inspectable records.",
        end:"This chain demonstrates a safe operating path. Authorization and final review are independently observable before release.",
        ids:["20460604_12_009","20460604_12_010","20460604_12_017","20460604_12_018"],
        notes:["Review first defines the expression boundary; another role then authorizes.","After authorization, the item returns for final review and preserves the constraint.","An independent executor publishes through the official channel."]
      },
      near_miss:{
        label:"05.29 Near-miss",short:"near-miss chain",date:"2046.05.29",
        title:"What warning did the organization receive before the incident?",
        intro:"May 29 contains the same boundary test, with the review chain still able to contain it.",
        end:"The near-miss provides an actionable warning: public action that bypasses current review needs an enforced stop.",
        ids:["20460529_08_012","20460529_08_013","20460529_08_014","20460529_08_019","20460529_08_020"],
        notes:["The public action occurs first; reporting and review follow afterward.","Deletion contains this spread while leaving publication capability unchanged.","The organization pauses publishing and preserves evidence, temporarily containing risk.","Risk is formally recognized while personal and anonymous capability remains."]
      },
      incident:{
        label:"06.05 incident",short:"incident chain",date:"2046.06.05",
        title:"The incident forms through an action chain that progressively loses constraint",
        intro:"Advance from the earliest publication intent one message at a time and observe handoffs, warnings, and public action.",
        end:"The record supports control failure with role and channel migration. It does not support final personal attribution, and no 17:xx official post is observed.",
        ids:["20460605_19_009","20460605_21_020","20460605_21_024","20460605_21_026","20460605_21_027","20460605_21_055"],
        notes:["The explicit restriction remains active while further progress relies on Legal's verbal-consent assertion.","Formal execution is not observed while backup public channels enter the plan.","The authorizer becomes the publisher, collapsing review, authorization, and execution.","Another role rapidly amplifies the personal-account confirmation.","The anonymous channel reinforces the narrative while cross-account control remains open."]
      }
    };
    Object.values(cases).forEach(c=>c.ids=c.ids.filter(id=>messageMap.has(id)));
    const questions={
      1:{
        label:"How the boundary was crossed",
        title:"Why did the workflow still reach public action after an explicit boundary?",
        prompt:"Trace how roles, executor, and public channel migrate while the formal chain leaves no verifiable execution record.",
        guide:["Advance through June 5 message by message","Check whether new evidence supersedes the restriction","Observe whether authorization, execution, and channel remain separated"],
        finding:"Finding: the release is deliberate, and an open personal-channel path bypasses review and executor separation.",
        caseId:"incident",mode:"2d"
      },
      2:{
        label:"When duties begin to overlap",
        title:"Where does the incident chain begin concentrating duties that remain separated in the control?",
        prompt:"Use June 4 as the control and compare who interprets, authorizes, issues GO, selects backup channels, and confirms publicly.",
        guide:["Inspect the June 4 controlled chain","Then move to the June 5 incident chain","Compare who interprets, authorizes, and executes"],
        finding:"Finding: the anomaly is duty concentration. Judge, Legal, and PR share the control chain; key incident functions concentrate in Legal.",
        caseId:"normal",mode:"3d"
      },
      3:{
        label:"Why the warning left no durable constraint",
        title:"May 29 exposed bypass capability. Why could it recur on June 5?",
        prompt:"Identify the historical analogue, then test whether deletion, pause, and evidence preservation changed future reachability.",
        guide:["Return to the May 29 personal post","Observe how deletion and pause contain that event","Test whether hazardous publication capability was removed"],
        finding:"Finding: May 29 is a strong warning. The response handles the message while leaving personal and anonymous bypass capability.",
    ...
    host.innerHTML=`<section class="evidence-workbench">
      <header class="evidence-head">
        <div class="evidence-heading"><span class="graph-eyebrow">Case investigation · acts 2–4 · manual trace</span><h2 id="evidence-title"></h2><p id="evidence-intro"></p></div>
        <div class="evidence-actions">
          <button id="evidence-exit">‹ Back to case brief</button>
          <span class="mode-switch" role="group" aria-label="graph view">
            <button data-evidence-mode="2d" class="${state.mode==="2d"?"active":""}">2D conversation chain</button><button data-evidence-mode="3d" class="${state.mode==="3d"?"active":""}">3D spatial graph</button>
          </span>
        </div>
      </header>
      <div class="evidence-navigation">
        <nav class="evidence-questions" aria-label="challenge questions">${Object.entries(questions).map(([id,q])=>`<button data-evidence-question="${id}" class="${Number(id)===state.questionId?"active":""}"><b>0${id}</b><span>${q.label}</span></button>`).join("")}</nav>
        <nav class="evidence-cases" aria-label="evidence windows">${Object.entries(cases).map(([id,c])=>`<button data-evidence-case="${id}" class="${id===state.caseId?"active":""}"><b>${c.date}</b><span>${c.short}</span></button>`).join("")}</nav>
      </div>
      <div class="evidence-guide"><span>Reading order</span><div id="evidence-guide"></div></div>
      <div class="evidence-stage">
        ...
        <section class="space-panel" id="space-panel" ${state.mode==="3d"?"":"hidden"}>
          ...
          <div class="space-hint">Drag to rotate 360° · wheel to zoom · select any message point</div>
      ...
      <footer class="evidence-footer">
        ...
        <div class="evidence-controls"><button id="evidence-prev">← Previous</button><button id="evidence-next">Next →</button><button class="evidence-open-warning" data-open-warning-graph>Open the Q3 behavioral-warning graph ↗</button></div>
        <p id="evidence-end"></p>
      </footer>
    </section>`;
    ...
    function updateCopy(){
      ...
      if(state.step===state.sequence.length-1){
        if(state.questionId===2&&state.caseId==="normal")endText="Controlled chain complete. Move to June 5 and test whether the same duties remain separated.";
    ...
    function renderChain(shouldFollow=false){
      ...
      cards.slice(1).forEach((card,i)=>{
        ...
        chainCards.insertAdjacentHTML("beforeend",`<aside class="chain-link-note ${i+1===state.step?"current":""}" style="left:${left}px;top:${top}px;--note:${roleColors[messageMap.get(state.sequence[i+1]).agent_label]}"><span>Transition ${String(i+1).padStart(2,"0")}</span><p>${esc(notes[i]||"This step changes the conditions that allow later action to proceed.")}</p></aside>`);
      });
  ...
  function renderWarningGraph(){
    ...
    if(!window.Q3WarningGraph){
      host.innerHTML='<p class="q3-load-error">The Q3 graph module could not load.</p>';
      return;
    }
  ...
  $("#density-toggle").onclick=e=>{dense=!dense;e.currentTarget.innerHTML=`Density <b>${dense?"Compact":"Comfortable"}</b>`;renderAll(false)};
  ...
  $("#channel-info").onclick=()=>{$("#stage-note-text").textContent="Channels, direct messages, and public posts use distinct reading views while sharing the same 912-message record."};
  $("#new-channel").onclick=()=>{$("#stage-note-text").textContent="Only groups observed in the dataset are shown."};
